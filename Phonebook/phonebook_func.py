...
def addToList(self):
    ...
    var_fname = var_fname.title()   # This will rensure that the first character in each word is capitalized
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):    # ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            if count == 0 or count2 == 0: # if proposed changes are not already n the database, then proceed
